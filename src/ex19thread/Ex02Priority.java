...
class MessageSendingThread extends Thread{
	...
	public void run() {
		for(int i=1; i<=1000 ; i++) {
			//각 쓰레드당 1000번씩 반복출력한다. 이때 우선순위를 각각 출력한다
...
public class Ex02Priority
{
	public static void main(String[] args)
	{
		...
		1단계실행
			...
			동일한 우선순위 5를 부여받게 된다. 이때 cpu가 알아서
			작업을 분해한다.
		 */
...
//		MessageSendingThread tr3 = new MessageSendingThread("세번째");
		...
		2단계 실행
			...
			1~10사이에 부여할수 잇고 높을수록 우선순위도 높아지게 된다.
...
//		MessageSendingThread tr3 = new MessageSendingThread("세번째", 1);
	...
	/*
	3단계 실행
		: 정수대신 이탤릭체인 static(정적) 상수를 사용하여 우선순위를 부여할수도
		있다. 결과는 2단계와 동일하다.
	 */
