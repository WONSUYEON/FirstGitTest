...
독립쓰레드 (Non Daemon Thread)
	...
	쓰레드를 말한다. 해당 쓰레드는 main()메소드가 종료된 이후에도 계속
	실행되는것을 볼수있다.
 */
...
class YesThread extends Thread
{
	...
	/*
	Thread클래스의 생성자를 호출할때 전달되는 매개변수를 통해 해당
...
/*
종속쓰레드(Daemon Thread)
	: 모든 독립쓰레드가 종료되면 자동으로 종료되는 쓰레드를 말하낟.
	메인메소드도 하나의 독립쓰레드 이므로, 다른 독립쓰레드가 남아있다면
	종료될때까지는 프로그램은 종료되는것이 아니다.
	주로 프로그램의 보조역할로 배경음악, 자동저장 
	구성한다. setDaemon(true)메소드로 
 */
...
class DaemonThread extends Thread
{
	...
	public void run(){
	while(true) {
		System.out.println(String.format("[쓰레드명:%s]째즈가 흘러요~", getName()));
	try {
		sleep(3000);
...
public class Ex03DaemonThread
{
	...
	/*
	첫번째 독립쓰레드(Non Daemon Thread) 생성
	 */
	YesThread yt1 = new YesThread("1st 쓰레드");//이름부여방법1
	//yt1.setName("첫번째 쓰레드");//이름부여 방법2
	yt1.start();
	
	//두번째 독립쓰레드 생성
	YesThread yt2 = new YesThread();//이름지정안했기때문에 자동으로 부여됨
