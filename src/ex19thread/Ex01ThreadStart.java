...
않아도 될때 사용하는 방식이다.
-만약 해당 클래스가 다른 클래스를 상속해야한다면 Runnable
인터페이스를 구현하여 사용한다.
 */
class ShowThread extends Thread
{
	...
	Thread클래스의 run()메소드를 오버라이딩 한것으로 해당메소드는
	...
	public void run()
	{
		for(int i=1;  i<=100 ; i++) {
			...
			try {
				/*
				Thread클래스의 static메소드로 실행의 흐름을 주어진
				시간만큼 block 상태로 전환시켜 준다.
...
public class Ex01ThreadStart
{
	...
	public static void main(String[] args)
	{
		...
		/*
		run()메소드를 통한 호출은 단순한 실행만 될뿐 
		쓰레드가 생성되지 않는다. 아래의 경우 첫번째 쓰레드객체의
		모든 작업이 완료된 후 두번째 쓰레드가 동작한다.
		 */
		//st1.run();
		//st2.run();
	}
