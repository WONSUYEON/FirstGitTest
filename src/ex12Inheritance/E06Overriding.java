...
오버라이딩(Overriding)
	...
	똑같은 형태로 재정의한다는 것이다.
	하위 클래스에서 오버라이딩한 매소드는 상위클래스의 메소드를
...
//일반적인 스피커의 기능에 중저음이 보강된 스피커를 추상화(상속받음)
...
public class E06Overriding
{
	public static void main(String[] args)
	{
		...
		/*
			자식객체로 호출하였으므로 자식클래스에서 오버라이딩한
			showState() 메소드가 호출된다. 부모쪽의 메소드는 
			가려지게 된다.
			*/
		...
		제한된다. 즉 setBase()는 자식쪽 멤버메소드 이므로 호출할 수 없다.
		 */
