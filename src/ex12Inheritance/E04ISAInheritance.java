...
class Computer{
	...
	/*
	private 접근지정자로 선언된 멤버메소드는 외부클래스 혹은 상속받은
	클래스에서 접근이 불가능하므로, public 으로 선언된 메소드를 통해
	간접 호출을 해야한다.
	상속관계라면 protected로 선언된 메소드로 호출할수도 있다.
	*/
...
class NotebookComputer extends Computer {
	...
	앞에서 다른 문장이 먼저 오면 오류가 발생되므로 주의해야한다.
	...
	public void movigCal() {
		...
		//private 으로 선언된 멤버메소드이므로 직접 호출할 수 없다.
		//keyboardTouch();//에러발생
		//calculate();//에러발생
		calculateFunc();
...
/*
태블릿 노트북 : 컴퓨터의 기본인 계산기능, 휴대이용 기능. 여기에
...
class TabletNotebook extends NotebookComputer{
	...
	public void write(String penInfo) {
		...
		/*
		A.compareTo(B)
			: 문자열A 와 B를 비교하여 0이 반환되면 같은 문자열로
