...
//자식클래스에서 확장한 메소드
	...
	자식쪽에서는 보이지 않아 오버라이딩의 대상이 
	될수 없다. 해당 함수는 자식쪽에서 새롭게 확장한 메소드가 된다.
	 */
	private void eat(){
	...
	그 보다 넓으면 가능하다.
	-부모에서 default 로 선언하면 자식에서는 default 혹은
	그보다 넓으면 오버라이딩이 가능하다.
	 */
	@Override
	public String sleep()
	...
	/*
	메소드 형태가 부모쪽과 다르므로(이건인자값이 있잖아 인트) 오버라이딩이 아님.
	다른 메소드이므로 오버로딩되어 확장한 메소드가 된다.
	 */
	...
	클래스 외부(메소드영역)에 로드되므로 오버라이딩 되지않고
