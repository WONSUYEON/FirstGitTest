...
abstract class AbsClass2{
	//추상메소드는 실행부 즉{} 를 쓰지않는다.
	abstract void absMethod(int number);
}
/*
위 클래스를 상속하면 부모가 가진 추상메소드를 그대로 상속받게 되므로
해당클래스도 abstract로 선언해야 한다. 단 이경우 해당 클래스로도
객체를 생성할 수 없게 된다.만약 객체를 생성해야 한다면
추상메소드를 자식에서 오버라이딩 하면 된다.
 */
...
public class E01AbstractBasic{
	...
	public static void main(String[] args){
		...
		/*
		추상클래스내에 선언된 정적멤버메소드는 오버라이딩의 
		대상이 아니므로 static의 기본규칙을 따른다. 즉 클래스명으로
		접근하면된다.
		 */
