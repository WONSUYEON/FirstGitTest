...
public class Ex05HashMapMain
{
	...
	{
		//Map컬렉션생성. key와 value는 둘다 String 형으로 선언.
		...
		int number = 20;
		//map.put("age", number);//[에러발생]->value타입이 맞지않아서
	...
	/*
	2. 키값을 모를때 : keyStet()메소드를 호출하여 키값을 Set컬렉션으로 
	얻어온다. 해당 반환값을 개선된 for문으로 추출하고, 이를 통해
	value를 얻어올수 있다. 
	 ...
	 */
	map.clear();
	System.out.println("전체삭제후 갯수:"+map.size());
	}
}
