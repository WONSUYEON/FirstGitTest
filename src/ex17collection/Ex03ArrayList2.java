...
public class Ex03ArrayList2
{
	...
	public static void main(String[] args)
	{
		...
		list2.add(st3);
		list2.add(st4);//인덱스3
		list2.add(st2);//리스트는 중복이 당연히 가능하죠.원빈객체 
		...
		for(Student st : list2) { //확장포문 사용
			st.showInfo();//4개 출력
		}
		System.out.println("[반복자=이터레이터 사용");
		Iterator<Student> it2 = list2.iterator();
		while(it2.hasNext()) {
			//Student student = it2.next();
			//student.showInfo(); 
			//위의 두문장을 한문장으로바꾸면 밑에코드 1줄이됩니다
			it2.next().showInfo();
		}
		
		//List 계열의 컬렉션은 인덱스로 접근가능함.
		...
		직접 접근할수 없다. getter/setter를 통해 접근해야한다.
